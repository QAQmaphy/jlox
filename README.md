...
```
源码 (String) → Scanner → Token[] → Parser → Stmt[] (AST) → Resolver → Interpreter → 求值结果
```

- **Scanner**（词法分析）: 将源码字符串转换为 Token 序列
- **Parser**（语法分析）: 递归下降解析，将 Token 序列构建为 AST
- **Resolver**（语义分析）: 遍历 AST 进行名称解析，通过作用域栈计算每个变量的深度并存入解释器，同时检测自引用、顶层 return 等语义错误
- **Interpreter**（解释求值）: 遍历 AST 执行语句和求值表达式
- **Environment**: 静态作用域，通过链式结构实现嵌套环境
- **Loxcallable**: 可调用对象的抽象接口，支持原生函数和用户定义函数
...
- **语句**: `var` 声明和初始化、赋值、`print`、表达式语句、`if`/`else`、`while`、`for`、函数声明（`fun`）、`return`
- **函数**: 用户定义函数、参数传递、闭包（捕获定义时环境）、原生函数（`clock`）
- **作用域**: 块 `{}` 创建嵌套作用域，变量遮蔽，内层可访问外层变量
- **语义分析**: Resolver 在运行前进行名称解析，检测 `var a = a;` 等错误，计算变量作用域深度
